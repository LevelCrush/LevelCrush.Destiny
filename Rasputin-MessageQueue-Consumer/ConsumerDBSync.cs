...
public static class ConsumerDBSync
{
        ...
        using (var db = await RasputinDatabase.Connect())
        {
            switch (message.Task)
            {
                case MessageDBSyncTask.MemberProfile:
                    result = await ProcessProfile(message.Data, db);
                    break;
                default:
    ...
    private static async Task<bool> ProcessProfile(string data, DBDestinyContext db)
    {
        ...
        if (profileResponse.Profile != null && profileResponse.Profile.Data != null)
        {
            ...
            tasks.Add(ProcessProfileComponent(profileResponse.Profile.Data, db));
        }
        ...
        if (profileResponse.Characters != null && profileResponse.Characters.Data != null)
        {
            ...
            tasks.Add(ProcessCharacterComponentMultiple(characters, db));
        }

        if (profileResponse.ProfileRecords != null && profileResponse.ProfileRecords.Data != null)
        {
            tasks.Add(ProcessTriumphComponent(membershipId, profileResponse.ProfileRecords.Data.Records, db));
        }
    ...
    private static async Task<bool> ProcessTriumphComponent(long membershipId,
        ConcurrentDictionary<string, DestinyRecordComponent> records, DBDestinyContext db)
    {
            ...
            .WhenMatched((old, @new) => new MemberTriumph()
                {
                    ...
                    UpdatedAt = ((DateTimeOffset)DateTime.UtcNow).ToUnixTimeSeconds(),
                })
            .RunAsync();
    ...
    private static async Task<bool> ProcessProfileComponent(DestinyProfileComponent profile, DBDestinyContext db)
    {
    ...
    private static async Task<bool> ProcessCharacterComponent(DestinyCharacterComponent character, DBDestinyContext db)
    {
    ...
    private static async Task<bool> ProcessCharacterComponentMultiple(DestinyCharacterComponent[] characters, DBDestinyContext db)
    {
        
        await Parallel.ForEachAsync(
            characters, 
            async (character,token) => await ProcessCharacterComponent(character, db)
            );
